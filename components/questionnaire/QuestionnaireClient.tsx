...
import { ALL_QUESTIONS, QUICK_MODE_QUESTION_IDS, PROFILES, calculateResult, type AnswerKey } from '@/app/questionnaire/data'
...
export default function QuestionnaireClient() {
  ...
  if (step === 'intro') return <ModeSelector onSelect={handleModeSelect} />

  if (step === 'result') {
    const profileId = calculateResult(answers)
    const profile = PROFILES[profileId]
    return <ResultCard profile={profile} answers={answers} onRestart={handleRestart} />
  }
    ...
    <div className="min-h-screen bg-q-bg flex flex-col">
      <header className="px-6 py-5 border-b border-q-border-sub">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <ThemeToggle />
            <button onClick={handleRestart} className="text-q-text-muted text-sm hover:text-q-text transition-colors">
              Questionnaire
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 flex items-start justify-center px-6 py-8">
        <div className="w-full max-w-3xl">
          <QuestionCard key={currentQuestion.id} question={currentQuestion} selectedAnswer={selectedAnswer} onAnswer={handleAnswer} />

          {currentIndex > 0 && !selectedAnswer && (
            <div className="mt-8" style={{ animation: 'fadeSlideIn 0.3s ease-out both' }}>
              <button onClick={handleBack} className="flex items-center gap-2 text-q-text-muted text-sm hover:text-q-text-sub transition-colors">
                <span>←</span>
                Question précédente
              </button>
            </div>
