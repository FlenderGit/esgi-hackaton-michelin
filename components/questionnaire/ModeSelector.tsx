        ...
        <div className="w-full max-w-2xl mx-auto space-y-10">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-q-yellow/20 bg-q-yellow/5">
              ...
              <span className="text-q-yellow text-xs font-medium tracking-widest uppercase">Quiz cycliste</span>
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-q-text leading-tight tracking-tight">
              Quel cycliste<br /><span className="text-q-yellow">es-tu ?</span>
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <ModeCard title="Version Rapide" questions={8} time="~2 min" description="Les questions essentielles pour cerner ton profil sans te prendre la tête." badge="Recommandé" onSelect={() => onSelect('quick')} />
            <ModeCard title="Version Complète" questions={20} time="~5 min" description="L'expérience intégrale pour une analyse en profondeur de ta personnalité cycliste." onSelect={() => onSelect('full')} />
          </div>
        </div>
      </main>
    </div>
...
function ModeCard({ title, questions, time, description, badge, onSelect }: ModeCardProps) {
  return (
    ...
    >
      {badge && (
        <span className="absolute top-4 right-4 px-2 py-0.5 rounded-full bg-q-yellow text-black text-[10px] font-black tracking-wider uppercase">{badge}</span>
      )}
      <div className="space-y-1">
        ...
        <h3 className="text-q-text font-bold text-xl group-hover:text-q-yellow transition-colors duration-200">{title}</h3>
      </div>
      <p className="text-q-text-sub text-sm leading-relaxed flex-1">{description}</p>
      <div className="flex items-center gap-2 text-q-text-muted group-hover:text-q-yellow transition-all duration-200 text-sm font-medium">
        <span>Commencer</span>
        <span className="translate-x-0 group-hover:translate-x-1 transition-transform duration-200 inline-block">→</span>
      </div>
    </button>
  )
}
